#-----------------------------------------------------------------------------
# Step 3: Установка header.cgi с полным меню
#-----------------------------------------------------------------------------
echo "${BLUE}Step 3: Installing header.cgi with full menu...${NC}"

# Создаем резервную копию
if [ -f /var/www/cgi-bin/header.cgi ]; then
    cp /var/www/cgi-bin/header.cgi /var/www/cgi-bin/header.cgi.original
    echo "  ✓ Original header.cgi backed up"
fi

# Скачиваем header.cgi из репозитория
...
# Проверяем что MQTT есть в меню
